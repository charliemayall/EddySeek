...
Session record types and shared plot/record primitives (JSON-serializable).
"""
...
class XYCloud:
    ...
    def probes(self) -> list[tuple[Offset, float]]:
        freq_row = self.freqs or (0.0,) * len(self.xs)
        return [
            (Offset(x, y), freq)
            for x, y, freq in zip(self.xs, self.ys, freq_row, strict=True)
        ]

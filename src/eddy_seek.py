...
try:
    ...
    from _eddy_seek.config import SeekConfig  # type: ignore[no-redef]
...
class EddySeek:
    def __init__(self, config) -> None:
        ...
        self._window_size: int = config.getint("window_size", 20)
    ...
    def cmd_EDDY_SEEK_SET(self, gcmd) -> None:
        ...
        if not changes:
            ...
            gcmd.respond_info(
                "EDDY_SEEK_SET: pass STRATEGY=ternary|centroid, TOLERANCE=…, etc. "
                "(overrides last until restart)"
            )
            return

        if any(c.startswith("window_size=") for c in changes):
            self._window_size = self.seek_config.window_size
